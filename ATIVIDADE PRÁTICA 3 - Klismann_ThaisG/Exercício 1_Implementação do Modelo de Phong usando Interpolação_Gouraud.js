...
material.vertexShader = `
    ...
    uniform vec3 Ip_position; //especular
    ...
    uniform vec3 k_a; // do termo ambiente
    uniform vec3 k_d; // do termo difuso
    uniform vec3 k_s; // do termo especular
    ...
    void main() {
        ...
        // Gouraud Shading (interpolação por vértice). 
           ...
           vec3 specular = Ip_diffuse_color * k_s * pow(max(vec3(0.0,0.0,0.0), dot(R_cam_spc, normalize(-L_cam_spc))), vec3(3.0,3.0,3.0));
            ...
            I = vec4(ambient + diffuse + specular, 1.0);
