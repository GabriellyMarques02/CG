//Código feito por Diego em 2021
...
material.vertexShader = `
    ...
    uniform vec3 Ip_ambient_color; // Ia
    uniform vec3 Ip_diffuse_color; // Ip
    ...
    float produtoInterno(vec3 vetor1, vec3 vetor2){
        float pInterno = dot(vetor1, vetor2);
        return max(0.0, pInterno);
    }
    ...
    void main() {
        ...
        vec3 L_cam_spc = normalize(Ip_pos_cam_spc.xyz - P_cam_spc.xyz); // l
        ...
        // Gouraud Shading (interpolação por vértice). 
           float n = 16.0;
           vec3 vetorQueApontaPraCamera = normalize(position - vec3(0,0,25)); //normalize(Ip_pos_cam_spc.xyz - position);
         
           vec3 gouraudShading = (
                                   Ip_ambient_color * k_a +
                                   Ip_diffuse_color * k_d * produtoInterno(L_cam_spc, N_cam_spc) +
                                   (Ip_diffuse_color * k_s) * pow(produtoInterno(R_cam_spc,vetorQueApontaPraCamera), n)
                                 );    
        ///////////////////////////////////////////////////////////////////////////////
        ...
        //I = vec4(1, 0, 0, 1);
        I = vec4(gouraudShading.x, gouraudShading.y, gouraudShading.z, 1);
